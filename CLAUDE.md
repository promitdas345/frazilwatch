...
that makes the final decision. Reasoning model is `granite-3-8b-instruct`.
...
- Reasoning model string: `watsonx/ibm/granite-3-8b-instruct`
- Granite emits a `<|tool_call|>` token before its JSON tool-call payload; if
  serving via vLLM, the parser flags are `--enable-auto-tool-choice` and
  `--tool-call-parser granite`. (Inside Orchestrate this is handled for you.)
